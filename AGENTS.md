...
When `.codex-harness.toml` is enabled and `mode = "control-plane"`, the harness rules take precedence over general development autonomy, `AGENTS.md` implementation workflow defaults, and generic skill guidance such as `superpowers:subagent-driven-development`.
...
- Implementation subagent prompts MUST stay minimal: task, exact write scope, scope boundaries, no-commit rule, no-revert rule, required validation commands, authorization/escalation reporting rule, and final report requirements. The final report should include completed work, changed files, validation results, and risks/open issues.
