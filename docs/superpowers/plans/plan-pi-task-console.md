...
| MCP 和工具权限 | MCP 接入方向已改为 `pi-mcp-adapter@2.8.0` package | 需要按当前 spec 重新实现 Task 7：固定 package 版本、启用 `directTools`、禁用 proxy、prewarm cache、保留多层 allowlist，并确保默认接入不依赖自维护 MCP client / 手写 schema | Task 7 |
...
- `packages/pi-task-console/src/types.ts`
- `packages/pi-task-console/src/tasks.ts`
- `packages/pi-task-console/src/task-store.ts`
- `packages/pi-task-console/src/task-dispatcher.ts`
- `packages/pi-task-console/src/run-manager.ts`
- `packages/pi-task-console/src/child-agent-process.ts`
- `packages/pi-task-console/src/env.ts`
- `packages/pi-task-console/src/server.ts`
- `packages/pi-task-console/public/index.html`
- `packages/pi-task-console/src/mcp-config.ts`
- `packages/pi-task-console/src/mcp-streamable-http-client.ts`
- `packages/pi-task-console/extensions/mcp-tools.ts`
- `packages/pi-task-console/.mcp.json`（如采用项目内示例配置）
- `packages/pi-task-console/.pi/mcp.json`（如采用项目内示例配置）
- `packages/coding-agent/package.json`（如引入或固定 package 依赖）
- `package-lock.json`（如依赖变更）
- `packages/pi-task-console/test/pi-task-console.test.ts`

新增：

- `packages/pi-task-console/scripts/run-police-workflow.mjs`
- `packages/pi-task-console/src/runtime-config.ts`
- `packages/pi-task-console/src/plan-validation.ts`
- `packages/pi-task-console/src/child-settings.ts`
- `packages/pi-task-console/src/prompt-builder.ts`
- `packages/pi-task-console/src/result-validation.ts`
- `packages/pi-task-console/src/persistence.ts`
- `packages/pi-task-console/public/styles.css`
- `packages/pi-task-console/public/app.js`
- `packages/pi-task-console/runtime.config.json`
- `packages/pi-task-console/runtime.config.example.json`
...
- 每次修改 `packages/pi-task-console/test/pi-task-console.test.ts` 后，从 `packages/pi-task-console` 运行：
...
| Gate 2 | Task 3、Task 4 完成后 | Gate 1 全部内容、runtime config、本地持久化、child RPC event、prompt 构造、最终结果解析和校验 | runtime config 可读可校验；`runtime.config.json` 与默认 env 契约一致；输出目录可配置且相对项目根目录解析；child Pi session 默认关闭；child settings 可写入或传递给 Pi RPC process；持久化不替代 `TaskStore`；prompt 包含 task 执行和 JSON 输出约束；`child_spawned` 触发 running；`prompt_response_failure` 触发 attempt fail；`prompt` success 不完成 task；`auto_retry_*` 只写日志不结算 attempt；tool error 只写日志不直接 fail attempt；unknown JSON event 进入 task logs 和 RPC event persistence；`agent_end.willRetry === true` 不结算 attempt；assistant `message_end` 是正常路径唯一 result 解析输入 |
| Gate 3 | Task 5、Task 6、Task 7 完成后 | Gate 2 全部内容、调度、retry、tool call limit、stop/replace、MCP package 接入和 allowlist | step 严格串行；step 内并发受配置限制；queued/unstarted task 在 stop/replace 后可标记 stopped 且不启动 child；attempts 用尽后 task 才最终 fail；attempt 终态释放 child process；tool call 超限可触发 retry 或最终 fail；kill 只发生在 steer 和 abort 等待之后；replace 不并发启动新旧 run；replace 后旧 run 迟到事件被忽略并记录诊断；`pi-mcp-adapter@2.8.0` 固定加载；`directTools` 生效；proxy `mcp` 不暴露；metadata cache prewarm 成功；task allowlist 多层强制生效；默认接入不依赖自维护 MCP client / 手写 schema；MCP/package 错误转 tool error 或明确启动错误 |
...
- 新增：`packages/pi-task-console/src/plan-validation.ts`
...
- [x] **Step 3: 使用 `tools` 表达 task 工具 allowlist**
...
- 修改：`packages/pi-task-console/src/types.ts`
- 修改：`packages/pi-task-console/src/task-store.ts`
...
`task-dispatcher.ts` 中生产 `TaskResult` 的 demo result 路径也必须使用 `data`，避免 store 消费侧和 producer 侧契约不一致。
...
- 新增：`packages/pi-task-console/src/runtime-config.ts`
- 新增：`packages/pi-task-console/src/child-settings.ts`
- 新增：`packages/pi-task-console/src/persistence.ts`
- 新增：`packages/pi-task-console/runtime.config.json`
- 新增：`packages/pi-task-console/runtime.config.example.json`
- 修改：`packages/pi-task-console/src/env.ts`
- 修改：`packages/pi-task-console/src/child-agent-process.ts`
- 修改：`packages/pi-task-console/src/run-manager.ts`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
...
- 修改：`packages/pi-task-console/.env.example`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
...
- 运行 `packages/pi-task-console/test/pi-task-console.test.ts`。
- 运行 `npm run check`。
...
- 修改：`packages/pi-task-console/src/child-agent-process.ts`
- 新增：`packages/pi-task-console/src/prompt-builder.ts`
- 新增：`packages/pi-task-console/src/result-validation.ts`
- 修改：`packages/pi-task-console/src/task-dispatcher.ts`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
...
**目标：** 按当前 spec 使用 `pi-mcp-adapter@2.8.0` package 接入 MCP，同时保留 Task Console subprocess RPC runtime 和 task `tools` allowlist 语义。
...
- 修改：`packages/pi-task-console/src/env.ts`
- 修改：`packages/pi-task-console/src/task-dispatcher.ts`
- 修改：`packages/pi-task-console/src/child-settings.ts`（如需要同步 Pi MCP adapter 配置到 child agent dir）
- 修改：`packages/pi-task-console/mcp.config.example.json`（如保留配置提示）
- 修改：`packages/pi-task-console/mcp.config.json`（如必须）
- 新增/修改：`packages/pi-task-console/.mcp.json`（如采用项目内示例配置）
- 新增/修改：`packages/pi-task-console/.pi/mcp.json`（如采用项目内示例配置）
- 修改：`packages/coding-agent/package.json`
- 修改：`package-lock.json`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
- 退出 active path：`packages/pi-task-console/src/mcp-config.ts`
- 退出 active path：`packages/pi-task-console/src/mcp-streamable-http-client.ts`
- 退出 active path：`packages/pi-task-console/extensions/mcp-tools.ts`
...
- [x] **Step 6: 默认 MCP 接入不依赖自维护 adapter**

要求：

- `mcp-config.ts`、`mcp-streamable-http-client.ts`、`extensions/mcp-tools.ts` 不再参与默认 MCP 接入链路。
...
- 修改：`packages/pi-task-console/src/server.ts`
- 修改：`packages/pi-task-console/src/run-manager.ts`
- 修改：`packages/pi-task-console/src/task-store.ts`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
...
- 修改：`packages/pi-task-console/public/index.html`
- 新增：`packages/pi-task-console/public/styles.css`
- 新增：`packages/pi-task-console/public/app.js`
- 修改：`packages/pi-task-console/src/server.ts`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
...
- 新增：`packages/pi-task-console/scripts/run-police-workflow.mjs`
- 修改：`packages/pi-task-console/src/task-store.ts`
- 修改：`packages/pi-task-console/src/task-dispatcher.ts`
- 修改：`packages/pi-task-console/src/child-agent-process.ts`
- 修改：`packages/pi-task-console/src/persistence.ts`
- 修改：`packages/pi-task-console/src/types.ts`
- 修改：`packages/pi-task-console/src/tasks.ts`
- 修改：`packages/pi-task-console/public/index.html`
- 修改：`packages/pi-task-console/public/app.js`
- 修改：`packages/pi-task-console/public/styles.css`
- 修改：`packages/pi-task-console/src/server.ts`
- 修改：`packages/pi-task-console/test/pi-task-console.test.ts`
- 复核：`logs/`
...
- `packages/pi-task-console/scripts/run-police-workflow.mjs` 读取公安 workflow JSON，并向 `/runs/start` 发送 `steps + userInstruction`。
- 前端“测试”按钮读取当前指令输入框内容，用公安 workflow JSON 触发 `/runs/start`。
- 指令输入框默认文本与 `packages/pi-task-console/scripts/run-police-workflow.mjs` 默认 `userInstruction` 一致。
...
- 默认 MCP 接入链路不依赖 `mcp-config.ts`、`mcp-streamable-http-client.ts`、`extensions/mcp-tools.ts`。
...
```bash
cd packages/coding-agent
npm --prefix packages/pi-task-console run dev
```
...
```bash
npm --prefix packages/pi-task-console run run:police-workflow
```
